...
export class CustomCacheService {
  ...
  private registerCache(instance: object) {
    return (methodName: string) => {
      ...
      const options = this.reflector.get<CustomCacheOptions>(CUSTOM_CACHE, methodRef);
      if (!options) {
        return;
      }
  ...
  async setCache({ options, args, result: _result, customKey }: { options: CustomCacheOptions; args: unknown[]; result: () => Promise<unknown>; customKey: string }) {
