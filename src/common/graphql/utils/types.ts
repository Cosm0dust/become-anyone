...
export type OperatorType<T> = TNe | TLt | TLte | TGt | TGte | TIn<T> | TNotIn | TContains | TNotContains | TNull | TNotNull | TBetween | TIContains | TNotIContains;

type ExtendedFindOptionsWhere<Entity> = {
  [P in keyof Entity]?: P extends "toString" ? unknown : FindOptionsWhereProperty<NonNullable<Entity[P]>> | OperatorType<Entity> | Entity[P] | ExtendedFindOptionsWhere<Entity>;
};

export type IWhere<T> = ExtendedFindOptionsWhere<T> | ExtendedFindOptionsWhere<T>[];
